<template>
    <div>
      <h1>欢迎加入豆瓣</h1>
       <form >
           <div class="username">
             <input type="text" placeholder="邮箱"  v-model="youxiang">
           </div>
           <div class="userpass">
              
                <input id="pasinp" type="text" placeholder="密码"  v-model="upass" >
             <span @click="imgbool=!imgbool" v-bind:class="imgbool?'cgreen':''" ></span>
             
           </div>
           <div class="formname">
               <input type="text" placeholder="用户名" v-model="uname"> 
           </div>
           <div class="buttons">
               <!-- <button type="button"  class="submit" @click="fun()" v-if="bool">立即注册</button>
               <button type="button"  class="submit" v-else>正在提交</button> -->
               <button type="button"  class="submit" @click="fun()"  v-if="bool">立即注册</button>
               <button type="button"  class="submit tijiao" v-else>正在提交...</button>
           </div>
        </form>
        <div class="footer">
            <div class="more-login">
                点击「注册」代表你已阅读并同意用户使用协议
            </div>
            <div class="btns">
                <a href="#">下载豆瓣APP</a>
            </div>
        </div>
    </div>
</template>
<script>
export default {
    data(){
        return{
            uname:"",
            upass:"",
            youxiang:"",
            bool:true,
            imgbool:false
        }
    },
    methods:{
        fun(){
            // console.log(this.uname+"---"+this.upass+"---"+this.youxiang)
            this.bool=false;
            var param=new URLSearchParams;
            param.append("uname",this.uname)
            param.append("upass",this.upass)
            param.append("youxiang",this.youxiang)
            this.axios({
                method:"post",
                url:"http://localhost:3000/zhuce",
                data:param,
             }).then((ok)=>{
                //  console.log(ok.data.status)
                if(ok.data.status==200){
                    alert("注册成功")
                    this.$router.push("/index");
                }else if(ok.data.status==100){
                    alert("用户名已经注册")
                    this.bool=true
                }else{
                      alert("注册失败")
                }
             })
            
        },
        funa(){

        }
    }
}
</script>
<style scoped>
   h1{
       margin: .3rem 0 .3rem;
       font-size: .4rem;
       font-weight: 300;
        color: #42bd56;
        text-align: center;
   }
    form{
        font-size: 0;
        padding: .2rem .18rem;
    }
    div>input{
        width: 95%;
        height: .44rem;
        padding: 0 .08rem;
        font-size: .15rem;
        border: .01rem solid #ccc;
       border-top-left-radius: .03rem;
        border-top-right-radius: .03rem;
        outline: 0;
    }
    .submit{
        cursor: pointer;
        width: 100%;
        padding: .12rem .26rem;
        margin-top: .1rem;
        font-size: .17rem;
        text-align: center;
        color: #fff;
        background: #17aa52;
        border: .1rem solid #17aa52;
        border-radius: .03rem;
        line-height: .01rem;
        outline: 0;
    }
    .tijiao{
            cursor: not-allowed;
            background: #eee;
             border: .1rem solid #eee;
    }
    div{
        font-size: 0;
    }
    .more-login{
        text-align: center;
        font-size:.15rem;
        color: #aaa;
    }
    .btns{
        margin-top: .4rem;
        text-align: center;
        font-size: .15rem;
    }
    .btns>a{
        color: #42bd56;
        margin-right: .15rem;
    }
    .userpass{
        position: relative;
        height: .4rem;
    }

    .userpass>input{
        position: absolute;
    }
    .userpass>span{
        display: inline-block;
        position: absolute;
        right: .01rem;
        /* right: -3rem; */
        width: .44rem;
        height: .44rem;
        line-height:.44rem;
         background: url(data:image/png;base64,)no-repeat;
        background-size: .22rem;
        background-position: 50%;
    }
    .userpass>.cgreen{
        display: inline-block;
        position: absolute;
        right: .01rem;
        width: .44rem;
        height: .44rem;
        line-height:.44rem; 
        background: url(data:image/png;base64,)no-repeat;
           background-size: .22rem;
        background-position: 50%;
}
</style>